...
for i = 0:(2*r-1),
    for j = 0:(2*r-1),
        if ((i >= r) && (j >= r)),
        H(i+1, j+1) = tempTerms(i+1, 1)*tempTerms(j+1, 1)*(t1^(i+j-2*r+1) - t0^(i+j-2*r+1))/(i+j-2*r+1);
        end
    end
end

% flip H so that it'll correspond to the order of c coefficients
H = rot90(rot90(H));
